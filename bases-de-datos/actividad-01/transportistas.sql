...
/* Punto 08 */
SELECT COUNT(RES.ID_CAMIONEROS) AS CANTIDAD_DE_PAQUETES, RES.CAMIONERO, RES.CIUDAD
FROM
(
	SELECT PAQ.ID_CAMIONEROS, CAM.NOMBRE AS CAMIONERO, PAQ.ID_PAQUETE, CIU.NOMBRE AS CIUDAD
	FROM PAQUETE PAQ
	INNER JOIN CAMIONEROS CAM
	ON PAQ.ID_CAMIONEROS = CAM.ID_CAMIONEROS
	INNER JOIN CIUDADES CIU
	ON PAQ.ID_CIUDADES = CIU.ID_CIUDADES
	WHERE CIU.NOMBRE = 'SALTA'
) AS RES
GROUP BY RES.ID_CAMIONEROS
ORDER BY RES.CAMIONERO ASC;
